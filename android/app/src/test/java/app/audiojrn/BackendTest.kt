...
class BackendTest {
    ...
    @Test fun task_create_sends_completed_in_the_single_create_request() = runBlocking {
        val server = MockWebServer(); server.start()
        server.enqueue(MockResponse().setBody("{\"text\":\"Terminada\",\"completed\":true}"))
        val repo = RetrofitBackendRepository(); val url = server.url("/").toString()

        repo.createTask(TaskItemCreate(id = "task-id", text = "Terminada", completed = true), url)

        val request = server.takeRequest()
        assertEquals("POST", request.method); assertEquals("/tasks", request.path)
        assertTrue(request.body.readUtf8().contains("\"completed\":true"))
        server.shutdown()
    }
