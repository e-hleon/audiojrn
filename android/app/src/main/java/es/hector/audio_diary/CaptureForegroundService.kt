...
class CaptureForegroundService : Service() {
    companion object {
        const val ACTION_START_MANUAL = "es.hector.audio_diary.START_MANUAL"
        const val ACTION_START_CONTINUOUS = "es.hector.audio_diary.START_CONTINUOUS"
        const val ACTION_START = ACTION_START_CONTINUOUS
        const val ACTION_PAUSE = "es.hector.audio_diary.PAUSE_CAPTURE"
        const val ACTION_RESUME = "es.hector.audio_diary.RESUME_CAPTURE"
        const val ACTION_FINISH = "es.hector.audio_diary.FINISH_CAPTURE"
        const val ACTION_STOP = ACTION_FINISH
        const val ACTION_DISCARD = "es.hector.audio_diary.DISCARD_CAPTURE"
        const val ACTION_PROCESS_NOW = "es.hector.audio_diary.PROCESS_CAPTURE_NOW"
    ...
    private fun migrateLegacyFinalizePreference() {
        val preferences = getSharedPreferences("audio_diary", MODE_PRIVATE); val session = preferences.getString("pending_finalize_session", null); val index = preferences.getInt("pending_finalize_last_index", -1)
