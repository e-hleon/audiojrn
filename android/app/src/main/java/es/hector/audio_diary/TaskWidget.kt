...
const val ACTION_WIDGET_TOGGLE = "es.hector.audio_diary.TOGGLE_TASK"
const val ACTION_OPEN_TASKS = "es.hector.audio_diary.OPEN_TASKS"
const val ACTION_OPEN_NEW_TASK = "es.hector.audio_diary.OPEN_NEW_TASK"
...
class AudioDiaryApplication : Application() {
    private val scope = CoroutineScope(SupervisorJob() + Dispatchers.Default)
    override fun onCreate() { super.onCreate()
        // Room invalidations update widgets; no timer or backend request is involved.
        scope.launch { TaskServices.repository(this@AudioDiaryApplication).visible.collectLatest { TaskWidgetProvider.refreshAll(this@AudioDiaryApplication) } }
    }
}
        ...
        if (intent.action == ACTION_WIDGET_TOGGLE) {
            ...
            CoroutineScope(Dispatchers.IO).launch {
                TaskServices.repository(context).let { repo ->
                    ...
                    repo.toggle(item); repo.schedule(context.getSharedPreferences("audio_diary", Context.MODE_PRIVATE).getString("backend_url", "").orEmpty())
                }
            }
        }
...
class TaskWidgetConfigActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: android.os.Bundle?) { super.onCreate(savedInstanceState)
        ...
        setContent {
            var hide by androidx.compose.runtime.remember { androidx.compose.runtime.mutableStateOf(getSharedPreferences("audio_diary", Context.MODE_PRIVATE).getBoolean(KEY_WIDGET_HIDE_DONE_PREFIX + id, false)) }
            MaterialTheme { Column(Modifier.padding(24.dp)) {
                ...
                Button(onClick = {
                    getSharedPreferences("audio_diary", Context.MODE_PRIVATE).edit().putBoolean(KEY_WIDGET_HIDE_DONE_PREFIX + id, hide).apply()
...
private class TaskFactory(private val context: Context, private val widgetId: Int) : RemoteViewsService.RemoteViewsFactory {
    ...
    override fun onDataSetChanged() { items = runBlocking(Dispatchers.IO) {
        val hide = context.getSharedPreferences("audio_diary", Context.MODE_PRIVATE).getBoolean(KEY_WIDGET_HIDE_DONE_PREFIX + widgetId, false)
