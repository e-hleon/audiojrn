...
class TaskLocalRepository(private val context: Context, private val database: TaskDatabase, private val backend: BackendRepository) {
    ...
    suspend fun delete(item: TaskItem) = database.withTransaction {
        ...
        (listOf(local) + dao.children(local.id)).forEach { target ->
            if (target.syncState == TaskSyncState.PENDING_CREATE) dao.remove(target.id)
            else dao.upsert(target.copy(syncState = TaskSyncState.PENDING_DELETE, updatedAt = now()))
        }
    }
    ...
    suspend fun sync(url: String): Boolean {
        ...
        try {
            ...
            dao.all().filter { it.syncState == TaskSyncState.PENDING_CREATE }.sortedBy { it.parentId != null }.forEach { local ->
                backend.createTask(TaskItemCreate(local.id, local.text, local.groupName, local.parentId, local.sortOrder, local.dueAt, local.allDay), url)
                dao.upsert(local.copy(syncState = TaskSyncState.SYNCED))
            }
    ...
    fun schedule(url: String) {
        context.getSharedPreferences("audio_diary", Context.MODE_PRIVATE).edit().putString("backend_url", url).apply()
...
class TaskSyncWorker(context: Context, params: WorkerParameters) : CoroutineWorker(context, params) {
    override suspend fun doWork(): Result {
        val url = applicationContext.getSharedPreferences("audio_diary", Context.MODE_PRIVATE).getString("backend_url", "").orEmpty()
